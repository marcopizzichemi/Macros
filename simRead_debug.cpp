// compile with
// g++ -o ../build/simRead_debug simRead_debug.cpp `root-config --cflags --glibs` && cp structDictionary.C ../build/
// syntax
// simRead_base `ls out*`
...
int main (int argc, char** argv)
{
  ...
  for(int iEvent = 0; iEvent < 30 ; iEvent++)
  {
    ...
    std::cout << "---------------------------------------------------------------------------------" << std::endl;
    std::cout <<"Event " << iEvent  << ", totalEnergyDeposited = " << totalEnergyDeposited<< std::endl;
    for(int eEvent = 0; eEvent < energyDeposition->size(); eEvent++) //run on energy depositions and find in how many crystals energy was deposited
    {
      ...
      std::cout << "cry " <<  cry << " " << "time = " << energyDeposition->at(eEvent).DepositionTime << std::endl;
    ...
    std::cout << std::endl;
    std::cout << "crystals.size() = " << crystals.size()<< std::endl;
    //
    ...
    if(totalEnergyDeposited > 0.510)
    {
      if(crystals.size() == 2)
      {
        std::cout << "Event" << iEvent << " crystals[0] = " << crystals[0] << ", crystals[1] = " << crystals[1] << std::endl;
